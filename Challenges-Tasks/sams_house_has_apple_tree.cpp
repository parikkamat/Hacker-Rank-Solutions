...
void countApplesAndOranges(int s, int t, int a, int b, vector<int> apples, vector<int> oranges) {
     vector<int> temp_apple;
     vector<int> temp_orange;
     int count_apple=0;
     int count_orange=0;
     for(int i=0;i<apples.size();i++){
         int temp1 = a + apples[i];
         temp_apple.push_back(temp1);
     }
     
     for(int j=0;j<oranges.size();j++){
         int temp2 = b + oranges[j];
         temp_orange.push_back(temp2);
     }
     
     for(int i1=0;i1<temp_apple.size();i1++){
          
          if(s <= temp_apple[i1] && temp_apple[i1]<=t)
             count_apple++;
     }
     
     for(int j1=0;j1<temp_orange.size();j1++){
          
          if(s <= temp_orange[j1] && temp_orange[j1]<=t)
             count_orange++;
     }
